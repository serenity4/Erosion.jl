...
function Base.getproperty(primitive::UpliftPrimitive{T}, name::Symbol) where {T}
  name === :asymmetric_ridge && return primitive.data::@NamedTuple{contour::Patch{BezierCurve,3,Vector{Point{2,T}}}, skeleton::Patch{BezierCurve,3,Vector{Point{2,T}}}, uplift::T, radius::T}
...
function uplift(primitive::UpliftPrimitive{T}, p) where {T}
  @match primitive.type begin
    &UPLIFT_PRIMITIVE_ASYMMETRIC_RIDGE => begin
      ...
      line = Line(p′, p)
      p ≈ p′ && return uplift
      ℐ = filter!(x -> GeometryExperiments.coordinate(line, x) ≥ zero(T), intersect(line, contour))
      isempty(ℐ) && return zero(T)
...
uplift_falloff(distance, radius) = distance > radius ? zero(distance) : (one(radius) - (distance/radius)^2)^3

function uplift_map(primitive::UpliftPrimitive{T}, (nx, ny)) where {T}
  Δx, Δy = one(T) ./ ((nx, ny) .- one(T))
  grid = [Point{2,T}(i, j) for i in zero(T):Δx:one(T), j in zero(T):Δy:one(T)]
