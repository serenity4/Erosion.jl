struct TectonicBasedErosion{E,UP,P,T,U} <: ErosionModel{E}
  uplift::UP
  iterations::U
  speed::T
  stream_power::T
...
ftype(::TectonicBasedErosion{E,UP,P,T}) where {E,UP,P,T} = T
utype(::TectonicBasedErosion{E,UP,P,T,U}) where {E,UP,P,T,U} = U

TectonicBasedErosion(args...; execution = CPU(), kwargs...) = TectonicBasedErosion{typeof(execution)}(args...; execution, kwargs...)
TectonicBasedErosion{E}(uplift, args...; kwargs...) where {E} = TectonicBasedErosion{E,Float64}(uplift, args...; kwargs...)
TectonicBasedErosion{E,T}(uplift, args...; kwargs...) where {E,T} = TectonicBasedErosion{E,T,Int}(uplift, args...; kwargs...)
function TectonicBasedErosion{E,T,U}(uplift, iterations::Integer = 1;
    speed = 100.0,
    stream_power = 0.0005,
    uplift_factor = 0.01,
    smooth_factor = 0.01,
  ...
  TectonicBasedErosion{E,typeof(uplift),inverse_momentum_power,T,U}(uplift, iterations, speed, stream_power, uplift_factor, smooth_factor, Val(inverse_momentum_power), min_slope, scale, execution)
end
...
inverse_momentum_power(model::TectonicBasedErosion{<:Any,<:Any,P}) where {P} = ftype(model)(P)
...
function simulate!(maps::ErosionMaps, model::TectonicBasedErosion{CPU})
  ...
  @parallelize model.execution for i in one(nx):nx
    for j in one(ny):ny
...
function simulate!(maps::ErosionMaps, model::TectonicBasedErosion, point::GridPoint, (nx, ny))
  T = ftype(model)
  (; elevation, drainage, uplift) = maps
  i, j = point[]

  # Border nodes are fixed to zero (elevation and drainage)
  if i in (one(nx), nx) || j in (one(ny), ny)
    maps.new_elevation[point] = zero(T)
  ...
  stream = drained^T(0.8) * compute_slope(elevation, point, downstream, model, (nx, ny))^2
...
precipitation(model, (nx, ny)) = norm(2 .* model.scale ./ @SVector [nx, ny])
...
@inline function compute_slope(elevation, from::GridPoint, to::GridPoint, model, (nx, ny))
  from == to && return zero(ftype(model))
...
@inline function drainage_weight(elevation, point, neighbor, model, (nx, ny), p)
  ...
  iszero(denom) && return zero(ftype(model))
  compute_slope(elevation, neighbor, point, model, (nx, ny))^p/denom
end

function laplacian(elevation, point, (nx, ny), model)
  i, j = point[]
  lx = ly = zero(ftype(model))
  I = signed(utype(model))
  for k in I(-1):I(1)
    p = GridPoint(i + k, j)
    is_outside_grid(p, (nx, ny)) && continue
    fac = ifelse(iszero(k), I(-2), I(1))
    lx += fac * elevation[p] / (2 * model.scale[1] / nx)^2
  end
  for l in I(-1):I(1)
    p = GridPoint(i, j + l)
    is_outside_grid(p, (nx, ny)) && continue
    fac = ifelse(iszero(l), I(-2), I(1))
